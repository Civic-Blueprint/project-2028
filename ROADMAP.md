...
3a. **Exchange #24 D1 — Recruit one warm-channel generalist reviewer for Constitutional Ecology reframe.** Per [Exchange #24 §2.5.bis Option E hybrid](agent/exchanges/coordination-architecture-reframe-exchange.md#25bis-revised-recommendation-v21-may-27--option-e-hybrid) Lane 1, identify one reviewer the steward already knows or can reach in one degree who is *not* embedded in Disability Justice or reciprocity-tradition scholarship but is willing to read the riff and exchange as a thoughtful outsider. Adapt the [Reviewer Packet Template](docs/REVIEWER_PACKET_TEMPLATE.md) for this exchange's five testable claims (C1–C5) and the §2.4 epistemic-status table. Process per the [Reviewer-as-a-Round Convention §1](docs/REVIEWER_AS_A_ROUND_CONVENTION.md). Lane 1 satisfies the Adversarial Review Protocol §5 audience-portability and steelman-integrity standing questions; it does *not* satisfy the missing-perspectives function (that is Lane 2's job per TODO #3b). Estimated steward time: 1–2 hours. **Does not consume TODO #11** — that lane is for F1/F3/F4 reviewer recruitment under the bounded-governance workstream, this is a separate exchange under a separate workstream.
