...
**Provisional read (updated post-Path β):** **Path β complete in a single overnight session per steward direction.** The exchange's Round 2 epistemic-status table is the artifact the steward reads to decide what happens next. Three options: (a) commission Round 3 external review with a Disability Justice-grounded reviewer (agent recommendation, on the record); (b) hold the reframe at Path β indefinitely while letting it inform downstream work informally (essentially Path α with the testable claims now on record); (c) proceed to Path γ doctrine drafting while explicitly carrying the unsolved questions (the legitimacy question, the metabolization-operationalization gap, the named-thinker-engagement depth gap, the depoliticization critique of the sequencing inversion) into doctrine as named open questions. The agent's reasoning for recommending Round 3: the reframe is strong enough that the cost of waiting is small; the cost of Path γ producing a metabolization-failure-shaped doctrine is large.
...
**Why it's here, not in active tracks (updated post-Path β).** Path β is complete as Exchange #24. The riff is now at v2; the reframe has survived first-pass adversarial review but did not pass cleanly. Whether this graduates to an active track depends on the steward's decision among (a)/(b)/(c) above. If (a) Round 3 is commissioned, the riff stays here with Exchange #24 as the active artifact; if (b) hold at β, the riff stays here informing downstream work; if (c) proceed to γ, a new active track *Doctrine — Coordination Architecture* should be created with the unsolved questions explicitly listed as named-research-debt. The metabolization check (OQ#7) remains the immediately-operationalizable exception in all three cases and could be drafted as a small standalone doctrine module independent of the larger doctrine decision.
