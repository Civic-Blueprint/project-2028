...
- **Survival-quality label (`S` / `V` / `S-trivial`) — required for consequential or iterated runs.** Severity catches *findings*; it misses the failure where a claim survives by going *empty*. Have the blind adversary additionally classify the whole set as `(S)` well-founded · `(V)` survives-because-vacuous (an inert escape clause / qualifier did the work) · `(S-trivial)` falsifiable only because the scope was cut below the question that motivated it. A `V`/`S-trivial` blocks "survives" even at zero BLOCKING. Full definition + rules: [harness §5.1](../../agent/process/cross-lineage-review-harness-protocol.md#51-survival-quality-label--the-anti-goodhart-classifier-added-june-2026).
- **Iterating the check?** If you will run more than one revise→re-attack cycle, follow the [harness §6.1 loop discipline](../../agent/process/cross-lineage-review-harness-protocol.md#61-iterated-revise-loops--the-converge-until-it-passes-guard-added-june-2026): rotate the adversary family each cycle, **pre-set the stop before round 1** (never stop *because* it finally passed), and judge the `BLOCKING`-trajectory + survival-quality label, not the last-round snapshot. One or two sharp cycles + human judgment beat many automated ones.
