...
These are complementary jobs. The Problem Map diagnoses lock-in, maps dependencies, and identifies the structural conditions under which reform becomes possible. The Systems Framework takes that diagnosis and applies it to design: what should be built, what should be prevented, where the stuckness is weakest, and what happens when it breaks. For the commitments that guide both documents, see [Principles](PRINCIPLES.md).

This document is meant to be collaborative. It is written as an invitation to think, challenge, refine, and build. If you are reading this and thinking, _"Yes, but what about..."_ — that is the point.
...
**Diagnostic questions** ground the section in an honest account of _why_ problems persist:
...
A society where shelter is easier to create, not harder.
That likely means rethinking zoning, permitting, construction capacity, infrastructure coordination, and the incentives that govern local growth.
...
Systems that make care easier to access, easier to understand, and less punishing to navigate.
That includes not just treatment, but prevention, mental health, administration, care coordination, and trust.
...
Clearer, more flexible pathways into work, contribution, public service, and mastery.
A society should make it easier for people to become useful, not harder.
...
**Design default — replacement over addition (project-wide, for bounded-governance proposals):** When the project proposes a new rule, program, mandate, or oversight body for a critical system, the default is *replacement over addition* — repurpose or sunset an existing instrument rather than layering a new one on top of the existing stack. Any proposal that is net-additive carries the burden of proof, and must clear it with an explicit justification of one of three kinds: an **asymmetric-risk** case (the harm of under-acting plausibly dwarfs the harm of the addition — the standard the frontier-AI package is allowed to meet), an **irreducible-coordination** case (no existing instrument can be repurposed to do the work), or a **named, defended exception**. The reasoning: this map's own diagnosis is that *any* system with concentrated benefits, diffuse costs, opaque feedback, and durable rule-setting power tends to ratchet toward extraction (see [Problem Map](PROBLEM_MAP.md) Domains 4 and 10). A public-interest architecture that only ever adds is therefore the architecture most exposed to the capture dynamic it is meant to resist — the addition becomes someone's protected incumbency. Requiring additions to justify themselves against replacement is the structural guard. This default was promoted from a local finding to a project-level commitment in [Exchange #21 Round 5](agent/exchanges/government-overreach-ownership-ratchet-exchange.md#implications-for-the-roadmap); it applies wherever a proposal expands governance scope, not only in this domain.
...
A future where advanced intelligence is governed in ways that strengthen humanity rather than quietly displace it.
That likely includes public-interest compute, AI governance models, transparency standards, public tooling, and stronger legitimacy around high-impact deployment.
...
Food systems appear stable until they are stressed.
Underneath that stability is a fragile web of logistics, concentration, environmental exposure, and supply chain dependency.
...
Food systems that are not just efficient, but resilient.
That may involve diversification, redundancy, regional capacity, better logistics, and stronger public understanding of where fragility actually lives.
...
This framework is not meant to flatten disagreement.
It is meant to make disagreement more useful — and more strategic.

The [Problem Map](PROBLEM_MAP.md) says: here is why everything is stuck.
This document says: here is where the stuckness is weakest, and here is what happens when it breaks.
