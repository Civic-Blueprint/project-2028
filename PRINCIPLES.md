...
They are not exhaustive policy positions.  
They are the core commitments that should guide decisions, tradeoffs, and system design.

They are offered as a starting point for global conversation, not as a finished framework from any single tradition. They are shaped by the contexts in which they were written and must be challenged, adapted, and expanded through engagement with people and communities whose experiences differ from those of the original authors.
...
If technological progress dramatically increases productivity, its benefits should translate into greater freedom, security, and opportunity for the broader public.
...
There is no single correct model for a good society. Different communities, cultures, and nations may legitimately organize themselves in different ways.

This project defines shared commitments, not a uniform blueprint. Self-determination and cultural diversity are features to be protected, not problems to be overcome. Better systems must be adaptable to diverse contexts, not exported as universal templates.
...
We are aware that principles without implementation are inert, that open processes can be captured, and that the communities most affected by systemic failure are often the least represented in projects like this one. These are not objections we dismiss — they are problems we commit to solving as part of the work itself.

The measure of this project is not the elegance of its principles, but whether they lead to systems that demonstrably improve lives.