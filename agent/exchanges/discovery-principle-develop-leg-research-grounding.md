...
## 9. Reviewer-surfaced sources (Round 2 — named, not yet grounded)

The reserved cross-lineage **Round 2** (GPT, Grok, Gemini; see [Exchange #27 §2](discovery-principle-develop-leg-exchange.md)) surfaced sources not in this digest. Named per the [§2 W8 discipline](../process/research-protocol.md) (not retrieved or tiered in this pass), to be grounded **only if a v2 claim set is ever opened** (the exchange currently holds at Round 2):

- **Kahan — identity-protective cognition** *(bears on S10)*: beliefs tied to group membership resist factual correction; named by GPT and Gemini as the better-evidenced cousin of the coupling claim.
- **Bandura — social learning / observational learning** *(counter to S2)*: moral architecture is also built via modeling and socialization, without encountered tension.
- **Piaget — disequilibrium** *(antecedent to S1/S2; strengthens the S18 relabel case)*.
- **Latané — social impact theory** *(counter to S12)*: raw number and immediacy of authorities do affect compliance.
- **Durkheim — anomie** *(counter to S12)*: non-correlated authorities can yield anomie/apathy rather than synthesis.
- **Rogers — client-centered therapy** *(S23 relabel target)*: the "therapist pole" maps onto non-directive client-centered practice.
- **Kohlberg / Rest — Defining Issues Test intervention literature** *(relabel case for moral-dilemma discussion, S2)*.
- **The moral-progress literature** *(bears on S17)*: collective moral change is driven by institutional, economic, and political conditions at least as much as by transferable individual capacity.
